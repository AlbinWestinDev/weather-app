...
class Forecast extends React.Component {
  ...
  getForecast = async (city, Api_Key) => {
    const result = await fetch(
      ...
      `http://api.openweathermap.org/data/2.5/forecast/?q=${city}&appid=${Api_Key}`
      // `http://api.openweathermap.org/data/2.5/forecast/daily?q=${city}&cnt=${cnt}&appid=${Api_Key}`
    );
        ...
        items.push(
          <div className="container text-light">
            <div className="Card">
              ...
              <h4 className="py-2">{Math.floor(item.main.temp - 273.15)} C°</h4>
              <h4 className="py-2">{item.weather[0].description}</h4>
            </div>
          </div>
        );
      }
